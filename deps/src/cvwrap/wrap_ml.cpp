...
define_ml_module(Module& mod) {
  ...
  mod.add_type<cv::ml::ParamGrid>("ParamGrid")
    ...
    .method("minVal",[](const cv::ml::ParamGrid &instance){return instance.minVal;})
    .method("maxVal",[](const cv::ml::ParamGrid &instance){return instance.maxVal;})
    .method("logStep",[](const cv::ml::ParamGrid &instance){return instance.logStep;});
  ...
  mod.method("randMVNormal", [](const  cv::Mat & mean,const  cv::Mat & cov,int  nsamples,const  cv::Mat & samples) {
    return cv::ml::randMVNormal( cv::InputArray(mean),cv::InputArray(cov),nsamples,cv::OutputArray(samples) );
  });
  mod.method("createConcentricSpheresTestSet", [](int  nsamples,int  nfeatures,int  nclasses,const  cv::Mat & samples,const  cv::Mat & responses) {
    return cv::ml::createConcentricSpheresTestSet( nsamples,nfeatures,nclasses,cv::OutputArray(samples),cv::OutputArray(responses) );
  });
